...
int main(int argc, char *argv[])
{
  ...
  cout << "edfsignals " << hdr.edfsignals << endl;
  ...
  printf("number of pnnotations in the file: %I64d\n", hdr.annotations_in_file);
#else
  ...
  cout << "EDFLIB_TIME_DIMENSION " << EDFLIB_TIME_DIMENSION << " double)hdr.file_duration " << (double)hdr.file_duration << " EDFSEEK_SET "  << EDFSEEK_SET <<" ((double)hdr.signalparam[channel].smp_in_file) " << ((double)hdr.signalparam[channel].smp_in_file) << endl;
   ...
   edfseek(hdl, channel, 1, EDFSEEK_SET);
...
n = edfread_physical_samples(hdl, channel, k, buf);
